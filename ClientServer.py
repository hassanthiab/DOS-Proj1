    ...
    def get(self, topicreq):
        ...
        if redis_cache.exists(topicreq):
            ...
            if y == 0:
                ...
                response = requests.get("http://" + BASE + ":8001/" + "search/" + topicreq)
                re = json.dumps(response.json())
                redis_cache.set(topicreq, re)
                y = 0
...
class Info(Resource):
    def get(self, idreq):
        ...
        if redis_cache.exists(idreq):
            ...
            if y == 0:
                response = requests.get("http://" + BASE + ":8000/" + "search/" + idreq)
                re = json.dumps(response.json())
                redis_cache.set(idreq, re)
                y = 1
            else:
                response = requests.get("http://" + BASE + ":8001/" + "search/" + idreq)
                re = json.dumps(response.json())
                redis_cache.set(idreq, re)
                y = 0
